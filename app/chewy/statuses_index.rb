...
class StatusesIndex < Chewy::Index
  ...
  settings index: index_preset(refresh_interval: '30s', number_of_shards: 5), analysis: {
    ...
    filter: {
      ...
      # 한국어 불용어 필터 - SNS 특화 (개인 검색용은 더 관대하게)
      korean_stop: {
        type: 'stop',
        stopwords: %w[
  ...
  root date_detection: false do
    ...
    field(:is_favourited_by_current_user, type: 'boolean', value: ->(status) { 
      # 현재 사용자가 즐겨찾기한 글인지 (인덱싱 시점에서는 알 수 없으므로 false)
      false 
    })
